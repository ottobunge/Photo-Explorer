...
class AppSettingsUpdate(BaseModel):
    ...
    thumbnail_quality: Optional[int] = Field(
        None, ge=1, le=100, description="JPEG quality for thumbnails (1-100)"
    )
    clip_model: Optional[str] = Field(
        None, min_length=1, max_length=100, description="CLIP model name"
    )
    face_detection_enabled: Optional[bool] = Field(None, description="Enable face detection")
    auto_index_new_photos: Optional[bool] = Field(
        None, description="Automatically index new photos"
    )

    # Performance settings
    thumbnail_cache_hours: Optional[int] = Field(
        None, ge=1, le=8760, description="Thumbnail cache duration in hours (1-8760)"
    )
    indexing_batch_size: Optional[int] = Field(
        None, ge=1, le=1000, description="Batch size for indexing (1-1000)"
    )
    indexing_parallel_workers: Optional[int] = Field(
        None, ge=1, le=32, description="Number of parallel workers (1-32)"
    )
    default_sync_interval_hours: Optional[int] = Field(
        None, ge=1, le=168, description="Default sync interval in hours (1-168)"
    )
