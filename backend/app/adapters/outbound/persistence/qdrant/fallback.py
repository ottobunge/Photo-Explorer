...
class QdrantFallbackQueue:
    ...
    async def queue_length(self) -> int:
        ...
        """
        return await self._redis.llen(self._queue_key)  # type: ignore[no-any-return]
    ...
    async def dequeue_batch(self, batch_size: int = 100) -> list[dict[str, Any]]:
        ...
        for _ in range(batch_size):
            task_json = await self._redis.lpop(self._queue_key)
            if task_json is None:
                break
            tasks.append(json.loads(task_json))  # type: ignore[arg-type]
        return tasks
